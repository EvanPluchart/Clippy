...
Téléchargez le DMG officiel depuis [clippy.evanpluchart.fr](https://clippy.evanpluchart.fr), ouvrez-le, puis glissez Clippy dans Applications.

![Installation de Clippy par glisser-déposer](Docs/Images/installer.jpg)

Clippy est gratuit et actuellement distribué sans signature Apple Developer ID ni notarisation. Gatekeeper peut donc bloquer le premier lancement :

1. Essayez une première fois d’ouvrir Clippy depuis Applications.
2. Ouvrez **Réglages Système → Confidentialité et sécurité**.
3. Descendez jusqu’à **Sécurité**, cliquez sur **Ouvrir quand même**, puis confirmez **Ouvrir**.

Téléchargez Clippy uniquement depuis le site officiel ou les [releases GitHub](https://github.com/EvanPluchart/Clippy/releases), et vérifiez si vous le souhaitez le checksum SHA-256 publié. Consultez les [instructions Gatekeeper d’Apple](https://support.apple.com/fr-fr/102445) pour plus de détails.
...
Le Cask installe le même DMG officiel. La confirmation Gatekeeper décrite ci-dessus peut donc être nécessaire une seule fois.
...
Le script crée une app universelle Apple silicon et Intel dans `dist/local/Clippy.app`. Elle est signée ad hoc et n’est pas notariée par Apple.
...
Clippy n’utilise volontairement pas App Sandbox, incompatible avec ce workflow Accessibilité autorisé par l’utilisateur. Hardened Runtime reste activé et l’app n’a aucun entitlement ou dépendance réseau.
...
Le chemin de release gratuit actuel documente explicitement son modèle de confiance :

```sh
./scripts/release_unsigned.sh
```

Il exige un dépôt Git propre, exécute les tests stricts, crée et valide une app universelle signée ad hoc, contrôle Hardened Runtime et les entitlements, fabrique le DMG, archive son dSYM, écrit un checksum SHA-256 et génère le Cask Homebrew correspondant. Il n’applique aucune signature Developer ID et n’envoie rien à Apple.

Le futur chemin signé reste disponible dans `scripts/release.sh`. Il nécessite un certificat Apple Developer ID et un profil de notarisation :
